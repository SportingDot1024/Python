...
def verifier():
	...
	if not ff.get() or ff.get() == "SELECT":
		...
		f = 1
	if not gg.get() or gg.get() == "SELECT":
		txt.insert(INSERT, "Gender  :  \n")
		g = 1
	if not hh.get() or hh.get() == "SELECT":
	...
	if a == 1 or admn == 1 or b == 1 or c == 1 or d == 1 or e == 1 or f == 1 or g == 1 or h == 1 or i == 1 or j == 1:
		return 1
	else:
		return 0
...
def verifier1():
	...
	if not ff1.get() or ff1.get() == "SELECT":
		txt.insert(INSERT, "Section  :  \n")
		f = 1
	if not gg1.get() or gg1.get() == "SELECT":
		txt.insert(INSERT, "Gender  :  \n")
		g = 1
	if not hh1.get() or hh1.get() == "SELECT":
		txt.insert(INSERT, "Stream Chosen  :  \n")
		h = 1
	if not ii1.get():
		txt.insert(INSERT, "DOB  :  \n")
	...
	if a == 1 or admn == 1 or b == 1 or c == 1 or d == 1 or e == 1 or f == 1 or g == 1 or h == 1 or i == 1 or j == 1 or k == 1:
		return 1
	else:
		return 0
		...
		for key, value in return_data.items():
			if key == rollNo:
				...
				txt.insert(INSERT, "Updated Details of Student -->>" + "\n\nAdmission No. : " + admn + "\nName : " +
						   return_data[key]["name_of_the_student"] + "\nFather's Name : " + return_data[key][
							   "father's name"] + "\nMother's Name : " + return_data[key][
							   "mother's name"] + "\nClass : " + return_data[key]["class"] + "\nSection : " +
						   return_data[key]["section"] + "\nGender : " + return_data[key]["gender"] + "\nStream : " +
						   return_data[key]["stream_chosen"] + "\nDOB : " + return_data[key]["dob"] + "\nMobile : " +
...
if __name__ == "__main__":
	...
	admn_admn = Entry(root, textvariable=admn, width=30)

	aa = ttk.Combobox(root, width=30)
	...
	ee = ttk.Combobox(root, width=30)
	ee['values'] = ("SELECT", 11, 12)
	ee.current(0)

	ff = ttk.Combobox(root, width=30)
	ff['values'] = ("SELECT", "A", "B", "C", "D", "E")
	ff.current(0)

	gg = ttk.Combobox(root, width=30)
	gg['values'] = ("SELECT", "Male", "Female")
	gg.current(0)

	hh = ttk.Combobox(root, width=30)
	hh['values'] = ("SELECT", "SCIENCE", "COMMERCE", "HUMANITIES", "ARTS")
	hh.current(0)

	ii = Entry(root, textvariable=dob, width=30)

	jj = Entry(root, textvariable=mob, width=30)

	kk = Entry(root, textvariable=ad, width=40)
	...
	admn_admn1 = Entry(root, textvariable=admn1, width=30)

	aa1 = ttk.Combobox(root, width=30)
	aa1['values'] = ("2020-21")
	aa1.current(0)
	...
	ee1 = ttk.Combobox(root, width=30)
	ee1['values'] = ("SELECT", 11, 12)
	ee1.current(0)

	ff1 = ttk.Combobox(root, width=30)
	ff1['values'] = ("SELECT", "A", "B", "C", "D", "E")
	ff1.current(0)

	gg1 = ttk.Combobox(root, width=30)
	gg1['values'] = ("SELECT", "Male", "Female")
	gg1.current(0)

	hh1 = ttk.Combobox(root, width=30)
	hh1['values'] = ("SELECT", "SCIENCE", "COMMERCE", "HUMANITIES", "ARTS")
	hh1.current(0)

	ii1 = Entry(root, textvariable=dob, width=30)

	jj1 = Entry(root, textvariable=mob, width=30)

	kk1 = Entry(root, textvariable=ad, width=40)
